Attribute VB_Name = "EC_Markup_Gen_new"
Option Explicit

Public Sub ec_markup_generator_new()
    ...
    '===============================================================================================================================================
    'Compare the reference BoM (imported) to the newly created BoM to check what ID numbers have changed part description, vendors, or vendor IDs
    '===============================================================================================================================================
    compare_BoMs ref_BoM_IDs, sb_start_row
            ...
            Select Case IsNumeric(Mid(ref_BoM_IDs(BoM_ID_counter, 1), 1, 1))                    'Check if the first Character is a number
                ...
                Case False                                                                      'If it is not a number
                    ...
                    Select Case ref_BoM_IDs(BoM_ID_counter, 2) = ""                             'Check if there is an ID number
                        ...
                        Case False                                                              'If there is an ID number
                            '=======================================================================================================================================
...
Private Sub compare_BoMs(ref_BoM_IDs, sb_start_row)
    Dim counter As Integer
    Dim current_row As Integer
    Dim markup_counter As Integer

    Dim rb_item_number As String
    Dim rb_description As String
    Dim rb_mfg As String
    Dim rb_mfg_id As String
    Dim sb_mfg As String
    Dim sb_mfg_id As String

    markup_counter = 0
    For counter = LBound(ref_BoM_IDs, 1) To UBound(ref_BoM_IDs, 1)
        current_row = sb_start_row + counter + markup_counter - 1
        rb_item_number = Trim(ref_BoM_IDs(counter, 1))
        rb_description = Trim(ref_BoM_IDs(counter, 6))
        rb_mfg = Trim(ref_BoM_IDs(counter, 9))
        rb_mfg_id = Trim(ref_BoM_IDs(counter, 14))
        sb_mfg = Trim(Range("I" & current_row).Value)
        sb_mfg_id = Trim(Range("N" & current_row).Value)

        '=======================================================================================================================================
        'If the reference book manufacturer or manufacturer ID is not the same as the newly created BoM:
            'Copy the new item and insert a row to get the formatting.
            'Create a strikethrough and make the text red on the item from the reference BoM and overwrite those values to the new BoM
            'Make the text blue on the new BoM to show that is what the value should be
        '=======================================================================================================================================
        If rb_mfg <> sb_mfg Or rb_mfg_id <> sb_mfg_id Then
            copy_paste_row current_row
            old_item current_row, rb_description, rb_mfg, rb_mfg_id, rb_item_number
            new_item current_row
            markup_counter = markup_counter + 1
        End If
    Next counter
End Sub
Private Sub copy_paste_row(current_row)
    Range("A" & current_row).Rows("1:1").EntireRow.Copy
    Range("A" & current_row).Offset(1, 0).Rows("1:1").EntireRow.Insert Shift:=xlDown
    Application.CutCopyMode = False
End Sub

Private Sub old_item(current_row, rb_description, rb_mfg, rb_mfg_id, rb_item_number)
    '=======================================================================================================================================
    'Create Strikethrough and red text
    '=======================================================================================================================================
    With Range("A" & current_row + 1).Rows("1:1").EntireRow.Font
        .FontStyle = "Regular"
        .Size = 8
        .Strikethrough = True
        .Superscript = False
        ...
        .TintAndShade = 0
    End With
    With Range("A" & current_row + 1).Rows("1:1").EntireRow.Interior
        .Pattern = xlNone
        .TintAndShade = 0
        .PatternTintAndShade = 0
    End With
    Range("A" & current_row + 1).Value = rb_item_number
    Range("F" & current_row + 1).Value = rb_description
    Range("I" & current_row + 1).Value = rb_mfg
    Range("N" & current_row + 1).NumberFormat = "@"
    Range("N" & current_row + 1).Value = rb_mfg_id
End Sub

Private Sub new_item(current_row)
    '=======================================================================================================================================
    'Make text blue
    '=======================================================================================================================================
    With Range("A" & current_row).Rows("1:1").EntireRow.Font
        .FontStyle = "Regular"
        .Size = 8
        .Strikethrough = False
        .Superscript = False
